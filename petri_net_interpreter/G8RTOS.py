...
from petri_net_interpreter.graph_entities \
...
class WifiDebug(Debug):
    ...
    def call(self, give_string):
        ...
        s += give_string + ';\n'
        s += self.get_debug_state()
        s += 'SendData((unsigned char*)(&debug_state), HOST_IP_ADDR,' + \
            str(self.string_len()) + ' );\n'
        s += self.parent.transition_sem.signal()
        s += '}\n'
        return s
...
class SharedBuffer(InterProccessCommunication):
    ...
    def prototype(self):
        ret = '#ifndef __SHARED_BUFFER_PROTOTYPES_' + \
            self.data_type + str(self.size()) + '__\n'
        ret += '#define __SHARED_BUFFER_PROTOTYPES_' + self.data_type + str(
            self.size()) + '__\n' + 'SharedBuffer_Prototypes(' + self.data_type + ',' + str(
            self.size()) + ')\n' + '#endif\n' + 'SharedBuffer_' + self.data_type + str(
            self.size()) + ' ' + self.name
    ...
    def define(self):
        ret = '#ifndef __SHARED_BUFFER_DEFINITIONS_' + self.data_type + str(
            self.size()) + '__\n' + '#define __SHARED_BUFFER_DEFINITIONS_' + self.data_type + str(
            self.size()) + '__\n' + 'SharedBuffer_Definitions(' + self.data_type + ', ' + str(
            self.size()) + ')\n' + '#endif\n'
        return ret
    ...
    def initialize(self):
        return 'SharedBuffer_Init_' + self.data_type + \
...
class G8RTOS(OperatingSystem):
    ...
    def add_thread(cls, place, param=''):
        ...
        ret = 'G8RTOS_AddThread((void(*)())(' + name + '), ' + \
            str(cls.priority()) + ', ' + '"' + name + '",\n'
