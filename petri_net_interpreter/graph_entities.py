...
class Edge(GraphEntity):
    ...
    def check_xlabel(self):
        if self.edge[1].attr['shape'] == self.parent.transition.shape():
            ...
            raise Exception(
                'Edge: %s must have a defined variable name to pass on' % str(
                    self.edge))
    ...
    def check(self):
        ...
        for i in shapes:
            if i != self.parent.place.shape() and i != self.parent.transition.shape():
                raise Exception(
                    "%s: '%s' is not a defined shape in this configuration.\n\
                         Current shapes are: %s" %
                    (self.edge, i, str(
                        {
                            'place': self.parent.place.shape(), 'transition': self.parent.transition.shape()})))

        # make sure graph is bipartite
        if shapes[0] == shapes[1]:
            raise Exception(
                "%s: Edge cannot connect two alike nodes" % str(
                    self.edge))
